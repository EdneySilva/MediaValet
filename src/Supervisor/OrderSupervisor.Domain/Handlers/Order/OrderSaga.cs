...
namespace OrderSupervisor.Domain.Handlers.Order
{
    public class OrderSaga : IHandler<CreateOrderCommand>
    {
        ...
        public async Task<ICommandResult> Handle(CreateOrderCommand command)
        {
            ...
            var response = await busMessageReply.WaitResponseAsync(command.OrderId.ToString(), TimeSpan.FromSeconds(10));
            return new MediaValet.Core.Commands.Results.SuccessResult
            {
                Data = response
            };
        }
